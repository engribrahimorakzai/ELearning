...
export const getCourseDetails = asyncHandler(async (req, res) => {
  ...
  // If user is student, include enrollment progress
  if (req.user) {
    const enrollment = await Enrollment.findByStudentAndCourse(req.user.id, req.params.id);
    if (enrollment) {
...
export const createCourse = asyncHandler(async (req, res) => {
  const { title, description, category_id, level, price, thumbnail_url, status } = req.body;
...
export const getCourseCurriculum = asyncHandler(async (req, res) => {
  ...
  // If user is logged in, get enrollment and completion status
  if (req.user) {
    const enrollment = await Enrollment.findByStudentAndCourse(req.user.id, req.params.id);
    if (enrollment) {
      ...
      // Mark completed lessons in curriculum
      curriculum.forEach(section => {
        if (section.lessons) {
          section.lessons.forEach(lesson => {
            lesson.is_completed = completedIds.includes(lesson.id);
          });
        }
      });
    }
